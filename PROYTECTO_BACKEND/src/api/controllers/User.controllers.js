...
const toogleUserMovie = async (req, res, next) => {
  try {
      ...
      if (userById) {
          ...
          const arrayIdMovies = movies.split(",");
          Promise.all(
            arrayIdMovies.map(async (movie, index) => {
                  if (userById.movies.includes(movie)) {
                      ...
                      try {
                          ...
                          try {
                              await Movie.findByIdAndUpdate(animal, {
                                  $pull: {
                                    likes: id
                                  },
                              });
              ...
              .then(async () => {
                  return res.status(200).json({
                      dataUpdate: await User.findById(id).populate("movies"),
                  });
              });
      } else {
          return res.status(404).json("este user no existe");
      }
  } catch (error) {
      return (
          res.status(404).json({
